...
(function($, win, undef) {
	...
	var tipmsg = { //默认提示文字;
		...
		w: {
			...
			"e": "邮箱地址格式不对",
			"url": "请填写网址",
			"phonePwd":"手机服务密码格式不正确",
			"imgcode":"图形验证码格式不正确",
			"msgcode":"短信验证码格式不正确",
			"homeAddress":"地址最多只能输入30个字符",
			"name": "姓名格式不正确",
			"companyName":"单位名称应由2-30个汉字组成",
			"companyPhone":"单位座机应由3-4位区号，中划线，7-8位数字组成。例：027-1234567",
			"formname": "与其他联系人姓名相同",
			"formphone": "与其他联系人手机号码相同",
			"idcard":"身份证号码格式不正确",
			"qq":"QQ号格式不正确",
			"bank":"请输入正确的银行卡号",
			"schoolName":"学校名称格式不正确",
			"specialty":"专业名称格式不正确",
			"idCardImg":"上传失败",
			"idCardPhoto":"上传失败",
			"chsiImg":"上传失败",
			"idcardOpposite":"上传失败",
			"dromAddress":"地址最多只能输入30个字符",
			"nation":"请选择民族"
		...
		nullmsg:{
			...
			"nation":"民族不能为空",
			"qq":"QQ号不能为空",
			"homeAddress":"详细地址不能为空",
			"m":"手机号不能为空",
			"phonePwd":"手机服务密码不能为空",
			"imgcode":"图形验证码不能为空",
			"msgcode":"短信验证码不能为空",
			"idCardImg":"请上传身份证正面照",
			"idcardOpposite":"请上传身份证背面照",
			"idCardPhoto":"请上传本人手持身份证照片",
			"bank":"银行卡号不能为空",
			"schoolName":"学校名称不能为空",
			"specialty":"专业名称不能为空",
			"dromAddress":"宿舍地址不能为空",
			"chsiImg":"请上传您的学历证明截图。如：录取通知书、学信网截图、毕业证书等",
			"companyName":"单位名称不能为空",
			"companyPhone":"单位座机不能为空"
	...
	Validform.util = {
		dataType: {
			"*": /[\w\W]+/,
			"homeAddress":/^[\w\W]{1,30}$/,
			"idCardImg":/[\w\W]+/,
			"idCardPhoto":/[\w\W]+/,
			"chsiImg":/[\w\W]+/,
			"idcardOpposite":/[\w\W]+/,
			"dromAddress":/^[\w\W]{1,30}$/,
			"*6-16": /^[\w\W]{6,16}$/,
			"n": /^\d+$/,
			"n6-16": /^\d{6,16}$/,
			"s": /^[\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]+$/,
			...
			"url": /^(\w+:\/\/)?\w+(\.\w+)+.*$/,
			"phonePwd":/^(\d{6,16})$/,
			"imgcode":/^[a-zA-Z | 0-9]{4,10}$/,
			"msgcode":/^(\d{6})$/,
			"companyName":/^[\u2E80-\uFE4F | a-zA-Z | 0-9]{2,30}$/,
			"companyPhone":/^([0-9]{3,4}-)?[0-9]{7,8}$/,
			"name": /^[\u2E80-\uFE4F]{2,8}$/,
			"qq":/^\d+$/,
			"bank":/^[0-9]{16,19}$/,
			"schoolName":/^[\u2E80-\uFE4F | 0-9 | a-zA-Z]{2,20}$/,
			"specialty":/^[\u2E80-\uFE4F | 0-9 | a-zA-Z]{2,20}$/,
			"nation":/^[0-9]*[1-9][0-9]*$/,
			"formname": function(gets, obj, curform, regxp) {
				var validflag = false;
				$(".formname").each(function(key, index) {
					if($(index).attr("name") != obj.attr("name")) {
						if($(index).val() == gets) {
							validflag = true;
							var msg = $(this).prev().html();
							// 与紧急联系人手机号相同，请核实后重新输入
							tipmsg.w.formname = "与" + msg + "相同，请核实后重新输入";
							return false;
						}
					}
				});
				if(validflag) {
					...
					tipmsg.w.formname = "与其他联系人姓名相同";
				}
			},
			"formphone": function(gets, obj, curform, regxp) {
				var validflag = false;
				$(".formphone").each(function(key, index) {
					if($(index).attr("name") != obj.attr("name")) {
						if($(index).val() == gets) {
							validflag = true;
							var msg = $(this).prev().html();
							...
							// 与紧急联系人手机号相同，请核实后重新输入
							tipmsg.w.formphone = "与" + msg + "相同，请核实后重新输入";
							return false;
						}
					}
				});
				if(validflag) {
					...
					tipmsg.w.formphone = "与其他联系人手机号相同";
				}
			},
			"idcard": function(gets, obj, curform, datatype) {
				...
				function isValidityBrithBy18IdCard(idCard18) {
					...
					var temp_date = new Date(year, parseFloat(month) - 1, parseFloat(day), 01, 00, 00);
		...
		_regcheck: function(datatype, gets, obj, curform) {
			...
			if(passed) {
				...
				if(Validform.util.isEmpty.call(obj, gets)) {
					...
					var temp = obj.attr("nullmsg");
					info = tipmsg.nullmsg[temp];
				}
			}
		...
		check: function(curform, subpost, bool) {
			...
			*/
			...
			//不是在提交表单时触发的ajax验证;
			if(inputval == this.validform_lastval && !$(this).attr("recheck") && subpost == "" && !$(this).hasClass("formname") && !$(this).hasClass("formphone")) {
